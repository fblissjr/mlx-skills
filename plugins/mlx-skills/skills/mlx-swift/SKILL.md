...
name: mlx-swift
description: >-
  MLX Swift: Apple's MLX machine learning framework for Swift on Apple
  silicon. Use when writing, debugging, or porting Swift MLX code -- array
  operations, neural networks, training, automatic differentiation, or custom
  Metal kernels. NOT the Python MLX framework (use the mlx skill) and NOT for
  running LLMs/VLMs in Swift (use the mlx-swift-lm skill). Triggers on "import
  MLX", "import MLXNN", "import MLXOptimizers", "MLXArray", "mlx-swift", "MLX
  Swift", "Swift MLX", "callAsFunction", "valueAndGrad", "@ModuleInfo",
  "UnaryLayer", "WiredMemory", "Apple silicon ML in Swift". Covers MLXArray
  operations, lazy evaluation, MLXNN layers and modules, optimizers,
  automatic differentiation, JIT compilation, custom kernels, wired-memory
  coordination, and Swift concurrency.
  Invoke with /mlx-skills:mlx-swift.
license: MIT
compatibility: "Requires macOS with Apple silicon (M1+) and Swift 5.9+ / Xcode 15+"
...
> **Related skills:** For the Python MLX framework, load the `mlx` skill. For
> running LLMs and VLMs in Swift, load the `mlx-swift-lm` skill.
